...
class LearnCppScraper:
    ...
    def save_tutorials(self, output_dir="learncpp_tutorials"):
        ...
        # Save individual chapters and lessons
        for chapter_num, chapter_data in self.tutorials.items():
            ...
            with open(os.path.join(chapter_dir, 'index.md'), 'w', encoding='utf-8') as f:
                ...
                for lesson_num, lesson_info in sorted_lessons:
                    safe_filename = f"{lesson_num} - {lesson_info['title']}.md"
                    safe_filename = re.sub(r'[^\w\s.-]', '', safe_filename)
                    f.write(f"- [{lesson_num} - {lesson_info['title']}]({safe_filename})\n")
            
            # Save individual lessons
            for lesson_num, lesson_info in chapter_data['lessons'].items():
                safe_filename = f"{lesson_num} - {lesson_info['title']}.md"
                safe_filename = re.sub(r'[^\w\s.-]', '', safe_filename)
                lesson_path = os.path.join(chapter_dir, safe_filename)
                
                with open(lesson_path, 'w', encoding='utf-8') as f:
                    f.write(f"# {lesson_num} - {lesson_info['title']}\n\n")
                    f.write(f"Source: {lesson_info['url']}\n\n")
                    if 'content' in lesson_info:
                        f.write(lesson_info['content'])
                logger.info(f"Saved lesson: {lesson_path}")
